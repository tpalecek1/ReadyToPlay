...
import 'package:ReadyToPlay/screens/home.dart';
...
class _SplashScreenState extends State<SplashScreen>
  ...
  void initState() {
    ...
    prepareWork().then((user) {
      if (user != null) {
        Navigator.of(context).pushReplacement(
            MaterialPageRoute(builder: (ctx) => Home(userId: user)));
      } else {
        Navigator.of(context).pushReplacement(
            MaterialPageRoute(builder: (ctx) => SignupLogin()));
      }
    });
  }
  ...
  Future prepareWork() async {
    await Future.delayed(Duration(seconds: 3),
        () {}); // can be removed... is just to show splash screen for at least 3 secs
